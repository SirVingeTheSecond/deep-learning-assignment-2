...
def load_data(normalize=True, augment_minority=True):
    ...
    visualize_dataset(train, val, test, class_names)
...
def create_dataloaders(x_train, y_train, x_val, y_val, batch_size, augment=False, balance=False):
    ...
    if balance:
        weights = [1.0 / Counter(y_train)[label] for label in y_train]
        sampler = WeightedRandomSampler(weights, len(y_train), replacement=True)
        shuffle = False

    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=shuffle, sampler=sampler, pin_memory=True)
    train_loader_clean = DataLoader(train_ds_clean, batch_size=batch_size, shuffle=False, pin_memory=True)
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False, pin_memory=True)

    return train_loader, val_loader, train_loader_clean
